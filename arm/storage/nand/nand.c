...
/* required buffers size */
#define SPARE_BUF_SIZE      (SPARE_SIZE + ECC_SIZE + 0x10)
#define CHIPID_BUF_SIZE     0x40
...
{
    if ((u32)chipid & 0x1f) {
        return nand_error("unaligned chipid buffer");
    }
    ...
    /* prepare for reading */
    dc_invalidaterange(chipid, CHIPID_BUF_SIZE);
    write32(NAND_CTRL, 0);
    write32(NAND_ADDR0, 0);
    write32(NAND_ADDR1, 0);
    write32(NAND_DATA, dma_addr(chipid));
    nand_irq_clear_and_enable();
    write32(NAND_CTRL,
        CTRL_FL_EXEC |
        CTRL_FL_IRQ |
        ...
        CTRL_CMD(CMD_CHIPID) | 
        CTRL_FL_RD | 
        CTRL_SIZE(CHIPID_BUF_SIZE));
    nand_wait_irq();

    if (read32(NAND_CTRL) & CTRL_FL_ERR) {
        return nand_error("error executing chipid read command");
    }
