...
#ifndef LOADER
static u32 __page_table[4096] ALIGNED(16384);
void _dc_inval(void);
...
void _ahb_flush_to(enum rb_client dev) {
    ...
    switch(dev) {
        ...
        default:
            ...
            //printf("_ahb_flush_to(%d): Invalid device\n", dev);
            return;
    }
...
void ahb_flush_to(enum rb_client dev)
{
    ...
    switch(dev) {
        ...
        default:
            printf("ahb_flush_to(%d): Invalid device\n", dev);
            return;
    }
...
void ahb_flush_from(enum wb_client dev)
{
    ...
    switch(dev)
    {
        ...
        case WB_ALL:
            ...
            break;
        default:
            printf("ahb_flush(%d): Invalid device\n", dev);
            goto done;
    }
    ...
    if(!done) {
        printf("ahb_flush(%d): Flush (0x%x) did not ack!\n", dev, req);
    }
done:
    irq_restore(cookie);
}
...
void mem_initialize(void)
{
    ...
    printf("MEM: cleaning up\n");

    _ic_inval();
    _dc_inval();
    _tlb_inval();

    printf("MEM: unprotecting memory\n");

    mem_protect(0, NULL, NULL);

    printf("MEM: mapping sections\n");
...
#ifndef NO_CACHES
    printf("MEM: enabling caches\n");

    cr |= CR_DCACHE | CR_ICACHE;
    set_cr(cr);

    printf("MEM: enabling MMU\n");
